# 🫧 Soap Bubble Interface — Complete Backup & Implementation Specification

> **Backup Date**: September 2, 2026  
> **Source Component**: `Urban/src/components/portfolio/IntroLoader.tsx`  
> **Purpose**: Standalone, fully reproducible record of the realistic soap bubble loading & intro screen with true glass transparency, iridescent film shimmer, organic wobble, sine-wave drift, multi-layer specular lighting, and particle burst physics.

---

## 1. Overview & Visual Architecture

The soap bubble interface is a two-phase entry screen for the 3D portfolio:
1. **Phase 1 (`loader`)**: Real-time progress bar connected to the VRM 3D avatar download hooks (`window.onVRMLoadProgress` and `window.onVRMReady`).
2. **Phase 2 (`bubbles`)**: Floating interactive soap bubbles that drift upward with lateral sine oscillation. Popping any bubble triggers:
   - 6 radial iridescent crack lines (`soapCrackAnim`).
   - 12 radial mist particles that expand and dissolve.
   - Smooth portfolio unveil + Raya AI voice greeting activation.

### Key Physics & Visual Features
- **True Transparency**: Near-zero body fill (`rgba(130, 220, 255, 0.04)` to `rgba(160, 120, 255, 0.10)`), allowing the dark space background to show through completely like real soap film.
- **Iridescent Film Hue Cycle**: `iridescentFilm` keyframe shifting `hue-rotate(0deg)` → `hue-rotate(360deg)` over 7s with unique per-bubble offset (`--shimmer-delay`).
- **Organic Wobble**: 5-point squish keyframes (`bubbleWobble`) with micro-rotations (±0.5°) and unique per-bubble timing (`--wob-dur`).
- **Lateral Sine Drift**: `floatSoapBubble` keyframe drifting ±22px laterally at 5 elevation waypoints with cubic-bezier easing.
- **Dual Specular Catch-Lights**:
  - **Primary**: Top-left soft ellipse (`::before`) with breathing pulse animation (`specularPulse`).
  - **Secondary**: Bottom-right crisp glint (`::after`) angled at 20°.
- **High-Fidelity Rim Lighting**: 8-layer inset box-shadow simulating light refraction, sub-surface scatter, and thin-film interference.

---

## 2. Complete Source Code (`IntroLoader.tsx`)
      ...
      {phase === 'bubbles' && (
        ...
        >
          ...
          <style>{`
            ...
            /* ── Outer halo glow breathe ── */
            @keyframes haloBreath {
              0%   { opacity: 0.48; transform: scale(1.00); }
            ...
            /* ════════════════════════════════════
               ...
               Strategy: near-zero fill, all visual
               information is in the rim + specular
            ════════════════════════════════════ */
            .soap-bubble-inner {
              /* Almost fully transparent — dark background shows through */
              background:
                /* Iridescent rim-only colour wash — very translucent */
                radial-gradient(
                  circle at 38% 36%,
                  transparent                30%,
              ...
              /* Razor-thin iridescent rim */
              border: 1.2px solid rgba(255, 255, 255, 0.50);

              /* All visual depth is in box-shadow layers:
                 1. Bright catch-lights (specular)
                 2. Thin coloured rim glow (iridescence)
                 3. Faint outer halo              */
              box-shadow:
                /* ── Primary specular: top-left catch-light ── */
                inset  9px  9px 18px rgba(255, 255, 255, 0.88),
                /* ── Top-center secondary fill ── */
                inset  0px  6px 10px rgba(220, 240, 255, 0.22),
                /* ── Bottom-right counter-light ── */
                inset -5px -5px 12px rgba(100, 220, 255, 0.18),
                /* ── Thin iridescent rim: pink → cyan colour band ── */
                inset  0    0   1px  rgba(255, 140, 220, 0.50),
                /* ── Very faint inner body tint ── */
                inset  0    0   28px rgba(180, 140, 255, 0.05),
                /* ── Outer glow: soft violet ring ── */
                0  0   6px  rgba(200, 120, 255, 0.22),
                /* ── Outer glow: distant bloom ── */
                0  0  18px  rgba(168,  85, 247, 0.12),
                /* ── Subtle drop shadow for ground depth ── */
                0 14px 20px rgba(0, 0, 0, 0.18);

              /* Thin glass: minimal blur so background is clearly visible */
            ...
            /* ── Primary specular: large soft ellipse top-left ── */
            ...
            /* ── Secondary specular: small glint bottom-right ── */
            ...
            /* ── Hover: brightens specular without adding fill ── */
...
## 3. How to Restore from this Backup

If you ever restore back to **Restore Point 3** (`restore-point-3`) and want to re-apply the enhanced transparent soap bubbles:

1. Replace `Urban/src/components/portfolio/IntroLoader.tsx` with the code above.
2. Rebuild the React bundle:
   ```powershell
   cd "Urban"
   npm run build
   cd ..
   Copy-Item -Path "Urban\dist\assets\*" -Destination "assets\" -Recurse -Force
   Copy-Item -Path "Urban\dist\index.html" -Destination "index.html" -Force
   ```
3. Commit locally:
   ```powershell
   git add -A
   git commit -m "feat(intro): apply backed-up transparent soap bubble interface"
   ```
